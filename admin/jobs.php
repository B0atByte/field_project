...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_all']) && $current_user_role === 'admin') {
    ...
    if (password_verify($password, $hashed)) {
        ...
        // ดึงข้อมูลงานที่จะลบก่อน เพื่อบันทึก log
        $sql_select = "SELECT * FROM jobs $where_clause";
        if (!empty($params)) {
            ...
            $result_jobs = $conn->query($sql_select);
        }
        $deleted_count = 0;
        ...
        // ปิด statement ถ้าใช้ prepared statement
        if (!empty($params) && isset($stmt_select)) {
            $stmt_select->close();
        }

        // ลบงานตามเงื่อนไข (ใช้ prepared statement)
        ...
        if (!empty($params)) {
            // มี parameters ต้องใช้ prepared statement
    ...
    <div class="filter-card">
      ...
      <form id="filtersForm" method="get" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="col-span-1 md:col-span-2 lg:col-span-4 flex flex-wrap gap-2 pt-2 border-t border-slate-100 mt-2">
          ...
          <button type="button" id="exportBtn" class="btn-secondary-action text-green-600 border-green-200 hover:bg-green-50">
...
$(document).ready(function () {
  ...
  const table = $('#jobsTable').DataTable({
    ...
    columns: [
      ...
      {
        data: 'created_at',
        render: (data) => `<div class="text-slate-600 text-sm">${esc(data)}</div>`
      },
      {
        data: 'officer_name',
        render: (data) => `<div class="text-slate-700">${esc(data)}</div>`
      ...
      },
      {
        ...
        render: (data) => {
          if (!data || data === '-') {
            return '<span class="inline-flex items-center px-2 py-1 text-xs font-medium text-orange-700 bg-orange-100 rounded-full">ยังไม่ส่ง</span>';
          }
          return `<div class="text-slate-600 text-sm">${esc(data)}</div>`;
        }
      },
      {
        data: 'actions',
  ...
  // 6) ปุ่ม Export – แนบตัวกรอง + คำค้นปัจจุบันไปด้วย
  $('#exportBtn').on('click', function () {
    ...
    Swal.fire({
      ...
      html: '<div class="flex items-center justify-center gap-3"><i class="fas fa-file-export fa-spin text-green-600 text-2xl"></i><span>กรุณารอสักครู่...</span></div>',
