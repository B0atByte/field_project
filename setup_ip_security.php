...
if ((isset($_SESSION['user']['role']) && $_SESSION['user']['role'] === 'admin') || in_array($currentIp, $localIPs)) {
    ...
    // ถ้าเข้าจาก local IP และยังไม่มี session user ให้สร้างขึ้นมา
    if (!isset($_SESSION['user']) && in_array($currentIp, $localIPs)) {
        $_SESSION['user'] = [
            'id' => 0,
            'name' => 'Local Admin',
            'username' => 'localadmin',
...
if (isset($_GET['invite'])) {
    $code = $conn->real_escape_string($_GET['invite']);
                ...
                <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-40">
                    <div class="px-6 py-4">
                        <h1 class="text-2xl font-bold text-gray-900 flex items-center">
                            <i class="fa-solid fa-shield-halved text-teal-600 mr-3"></i>
                            Security Gateway
                        </h1>
                        <p class="text-sm text-gray-600 mt-1">Device Registration & IP Access Control</p>
                    </div>
                </header>
                <main class="p-6">
                <?php else: ?>
                    <div class="min-h-screen flex flex-col justify-center py-6 px-4 sm:py-10">
                    <?php endif; ?>

                    <?php if (!$isSetupAllowed && !isset($_COOKIE['device_perm_token'])): ?>
                        <!-- ACCESS DENIED VIEW -->
                        <div
                            class="max-w-md mx-auto w-full bg-white rounded-xl shadow-sm border border-slate-200 p-6 sm:p-8 text-center">
                    ...
                    <?php else: ?>
                        ...
                        <div class="<?= $isAdmin ? '' : 'max-w-3xl mx-auto w-full' ?> space-y-6 sm:space-y-8">

                            <?php if (!$isAdmin): ?>
                                <!-- Logo / Header (Non-Admin) -->
                                ...
                                <div class="p-5 sm:p-8">
                                    ...
                                    <?php if ($myStatus === 'approved'): ?>
                                        ...
                                        <div class="text-center py-6">
                                            <div
                                                class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-emerald-50 mb-4">
                                    ...
                                    <?php elseif ($myStatus === 'pending'): ?>
                                        ...
                                        <div class="text-center py-6">
                                            <div
                                                class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-amber-50 mb-4">
                                            ...
                                            <h4 class="text-lg font-medium text-slate-900 mb-1">รอการอนุมัติ (Waiting for
                                                Approval)</h4>
                                            <p class="text-slate-500 text-sm">คำขอของคุณถูกส่งแล้ว กรุณารอ Admin อนุมัติสิทธิ์
                                            </p>
                                        </div>
                                        ...
                                        <?php if ($isSetupAllowed): ?>
                                            <form method="POST" class="space-y-4">
                                                <div>
                                                    ...
                                                    <input type="text" name="device_desc"
                                                        placeholder="e.g. Somchai iPhone, Site Laptop"
                            ...
                            <?php if ($isAdmin): ?>
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    ...
                                    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-5">
                                        <label class="block text-sm font-medium text-slate-700 mb-3">Create Invitation
                                            Link</label>
                                        <form method="POST" class="space-y-3">
                                            <button type="submit" name="gen_invite"
                                                class="w-full bg-teal-600 hover:bg-teal-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium shadow-sm transition-colors">
                                                <i class="fa-solid fa-link mr-2"></i>Generate Link (HTTPS)
                                            </button>
                                    ...
                                    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-5">
                                        <h4 class="text-sm font-medium text-slate-700 mb-2">Pending Requests</h4>
                                        <div class="text-3xl font-bold text-amber-600">
                                            <?= $pending ? $pending->num_rows : 0 ?>
                                        </div>
                                        <p c lass="text-xs text-slate-500 mt-1">รออนุมัติ</p>
                                    </div>
                                </div>
                                ...
                                <!-- All Approve d Devices -->
                                <div class="bg-white  rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                                    <div class="px-5 py-4 border-b border-slate-200">
                                        <h4 class="text-sm font-medium text-slate-900">Approved Devices</h4>
                                    </div>
                                    <ul class="divide-y    divide-slate-100 max-h-60 overflow-y-auto">
                                        ...
                                        while ($d = $allDev->fetch_assoc()):
                                            ?>
                                            <li clas s="px-4 py-3 flex justify-between items-center hover:bg-slate-50">
                                ...
                                <!-- Fixed I  Ps -->
                                <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-5">
                                    <h4 class="text-sm font-medium text-slate-900 mb-4">Fixed IPs (Server/Office)</h4>
                                    <form method="POST" class="flex flex-col sm:flex-row gap-2 mb-4">
                                        ...
                                        <button type="submit" name="add_manual"
                                            class="w-ful l sm:w-auto bg-slate-900 text-white px-4 py-2.5 rounded-lg text-sm hover:bg-slate-800 transition">Add</button>
                                    </form>
