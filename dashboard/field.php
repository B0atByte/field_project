...
function daysDiffBadge($due) {
  ...
  if (preg_match('/^-?\d+$/', $raw)) { $d = (int)$raw; }
  else {
    $formats = ['Y-m-d','Y-m-d H:i:s','d/m/Y','d-m-Y','m/d/Y'];
    $dueDate = null;
    foreach ($formats as $f) { $dt = DateTime::createFromFormat($f,$raw); if ($dt && $dt->format($f)===$raw) { $dueDate=$dt; break; } }
    if (!$dueDate) { $ts=strtotime($raw); if ($ts!==false) $dueDate=(new DateTime())->setTimestamp($ts); }
  ...
  if ($d === 0) return '<span class="status-chip warning">วันนี้</span>';
  if ($d <= 3) return '<span class="status-chip caution">อีก '.$d.' วัน</span>';
  return '<span class="status-chip success">อีก '.$d.' วัน</span>';
}

function priorityChip($p) {
  $p = $p ?: 'normal';
  if ($p==='urgent') return '<span class="priority-chip urgent">ด่วนที่สุด</span>';
  if ($p==='high')   return '<span class="priority-chip high">ด่วน</span>';
...
function buildUrl($params = []) {
  ...
  if ($merged['priority'] === 'all') unset($merged['priority']);
  if ($merged['status'] === 'active') unset($merged['status']); // active เป็นค่าเริ่มต้น
  if ($merged['favorite'] === '0') unset($merged['favorite']);
      ...
      <div class="px-4 py-4">
        <div class="flex items-center justify-between gap-3">
          <div class="flex items-center gap-3 min-w-0 flex-1">
            <div class="w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-sm">
          ...
          <div class="flex items-center gap-2">
            ...
            <!-- Menu Toggle -->
            <button id="menuToggle" class="p-2.5 rounded-xl bg-gray-50 hover:bg-gray-100 border border-gray-200 transition-all">
              <svg id="menuIcon" class="w-5 h-5 text-gray-600 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div id="collapsibleMenu" class="menu-container menu-collapsed">
          <div class="pt-4 space-y-4">
            ...
            <form method="GET" action="" id="searchForm">
              ...
              <div class="relative mb-3">
                <input id="searchInput" 
                       ...
                       placeholder="ค้นหาด้วย สัญญา / ชื่อ / โซน / ทะเบียน / ที่อยู่ / จังหวัด..." 
                       class="search-input">
            ...
            $stmt_count->execute();
            $returned_count = $stmt_count->get_result()->fetch_assoc()['total'];
            $stmt_count->close();
            ?>
            <div class="flex flex-col sm:flex-row gap-3 items-start">
              ...
              <div class="flex flex-wrap gap-2">
                ...
                <a href="<?= buildUrl(['status' => 'returned', 'page' => 1]) ?>"
                   class="filter-btn <?= $status_filter === 'returned' ? 'active' : '' ?>"
                   style="position: relative;">
                   งานที่ถูกตีกลับ
                   <?php if ($returned_count > 0): ?>
                   <span class="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center shadow-lg">
                     <?= $returned_count ?>
                   </span>
                   <?php endif; ?>
                </a>
            ...
            <div class="flex flex-wrap items-center gap-3">
              ...
              <select id="perPageSelect" class="px-4 py-2 border border-gray-200 rounded-xl bg-white text-sm font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
      ...
      <div id="cardsView" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        <div class="job-card p-5 animate-slide-in">
          ...
          <div class="flex gap-4">
            <button class="fav-btn <?= $row['is_favorite'] ? 'active' : 'inactive' ?> toggle-fav"
            ...
            <div class="flex-1 min-w-0">
              <div class="flex flex-wrap items-center gap-2 mb-3">
                <div class="font-bold text-gray-900 truncate text-lg"><?= htmlspecialchars($row['contract_number'] ?? '') ?></div>
          ...
          <table>
            ...
            <tbody>
              ...
              <tr class="animate-slide-in">
                ...
                <td>
                  <div class="flex items-center gap-2">
                    <span class="font-semibold text-gray-900"><?= htmlspecialchars($row['contract_number'] ?? '') ?></span>
                    <?php if ($isNew): ?><span class="status-chip danger animate-pulse-custom">NEW</span><?php endif; ?>
                  </div>
                </td>
