  ...
  <div class="flex flex-col flex-1 lg:ml-64 w-full">
        ...
        <div class="flex items-center justify-between gap-2">
          <div class="flex-1 min-w-0">
            <h1 class="text-base sm:text-xl font-semibold text-gray-900 flex items-center truncate">
              <svg class="w-4 h-4 sm:w-6 sm:h-6 mr-1 sm:mr-2 text-gray-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex items-center gap-2">
            ...
            ?>
            <span class="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium border <?= $status_colors[$job['status']] ?? 'bg-gray-100 text-gray-800 border-gray-200' ?>">
    ...
    <main class="flex-1 p-3 sm:p-6 space-y-4 sm:space-y-6 overflow-y-auto">
      ...
      <section class="flex flex-wrap justify-between items-center gap-2 sm:gap-3 animate-fade-in">
          ...
          <?php if (!$isFieldRole): ?>
            <?php if ($job['status'] === 'completed'): ?>
              ...
              <form action="../admin/export_job_detail_word.php" method="post">
                ...
                <button type="submit" class="inline-flex items-center justify-center px-3 sm:px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm sm:text-base font-medium transition-all duration-200 shadow-sm hover:shadow-md">
            ...
            <?php else: ?>
              <!-- Disabled Export -->
              <button onclick="alertExportDenied()" class="inline-flex items-center justify-center px-3 sm:px-4 py-2 bg-gray-400 text-white rounded-lg text-sm sm:text-base font-medium cursor-not-allowed">
                <svg class="w-4 h-4 mr-1 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                <span class="hidden sm:inline">Export Word</span>
        ...
        <a href="<?= $backUrl ?>" class="inline-flex items-center justify-center px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm sm:text-base font-medium transition-all duration-200 shadow-sm hover:shadow-md ml-auto">
          <svg class="w-4 h-4 mr-1 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <section class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 animate-slide-up">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
            <div class="flex items-center mb-2">
              <svg class="w-4 h-4 mr-2 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2m-9 4v10a2 2 0 002 2h8a2 2 0 002-2V8M7 8h10M7 8V6a2 2 0 012-2h6a2 2 0 012 2v2"/>
              </svg>
              <span class="text-sm font-medium text-gray-600">เลขที่สัญญา</span>
            </div>
            <p class="text-base font-semibold text-gray-900"><?= htmlspecialchars($job['contract_number']) ?></p>
          </div>
          ...
          <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <p class="text-base font-semibold text-gray-900"><?= htmlspecialchars($job['overdue_period']) ?></p>
          </div>
          ...
          <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <p class="text-base font-semibold text-gray-900"><?= number_format(floatval(str_replace(',', '', $job['os'])), 2) ?> บาท</p>
          </div>
      ...
      <section class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 animate-slide-up" style="animation-delay: 0.1s;">
          ...
          <div class="space-y-3">
            ...
            <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-2">
              <div class="flex justify-between">
                ...
                <span class="text-sm font-semibold text-gray-800"><?= htmlspecialchars($job['location_info']) ?></span>
              </div>
              <div class="flex justify-between">
                <span class="text-sm text-gray-600">เลขบัตรประชาชน:</span>
                <span class="text-sm font-semibold text-gray-800"><?= htmlspecialchars($job['customer_id_card']) ?></span>
              </div>
              <div class="flex justify-between">
                <span class="text-sm text-gray-600">ที่อยู่:</span>
                <span class="text-sm font-semibold text-gray-800"><?= htmlspecialchars($job['location_area']) ?></span>
              </div>
              <div class="flex justify-between">
          ...
          <div class="space-y-3">
            ...
            <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-2">
              ...
              <div class="flex justify-between">
                ...
                <span class="text-sm font-semibold text-gray-800"><?= htmlspecialchars($job['model_detail']) ?></span>
              </div>
              <div class="flex justify-between">
              ...
              <div class="flex justify-between">
                ...
                <span class="text-sm font-semibold text-gray-800"><?= htmlspecialchars($job['province']) ?></span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mt-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="flex justify-between">
              ...
              <span class="text-sm font-semibold text-gray-800"><?= htmlspecialchars($job['imported_name'] ?? '-') ?></span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-600">ผู้รับผิดชอบ:</span>
              <span class="text-sm font-semibold text-gray-800"><?= htmlspecialchars($job['assigned_name'] ?? '-') ?></span>
            </div>
          </div>
        </div>
      </section>
      ...
      <!-- Work Result -->
      <section class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 animate-slide-up" style="animation-delay: 0.2s;">
        <div class="mb-5">
          <h2 class="text-lg font-semibold text-gray-900 flex items-center">
            <svg class="w-5 h-5 mr-2 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-4">
          ...
          <?php if ($job['log_time']): ?>
            <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
              ...
              <p class="text-base font-semibold text-gray-900"><?= date('d/m/Y H:i น.', strtotime($job['log_time'])) ?></p>
            </div>
          <?php endif; ?>
      ...
      <?php if (!empty($job['images'])): ?>
        ...
        <?php if (is_array($images) && count($images) > 0): ?>
          <section class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 animate-slide-up" style="animation-delay: 0.3s;">
            <div class="mb-4 sm:mb-5">
              <h2 class="text-base sm:text-lg font-semibold text-gray-900 flex items-center">
                <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                รูปภาพประกอบ
                <span class="ml-2 text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full"><?= count($images) ?> รูป</span>
              </h2>
              <div class="w-12 h-0.5 bg-gray-300 mt-2"></div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 mb-4 sm:mb-5">
              ...
              ?>
                <div class="group relative bg-gray-100 rounded-lg overflow-hidden hover:shadow-md transition-all duration-200">
                  <!-- ทุก Role สามารถคลิกดูภาพได้ -->
                  <button type="button"
                     onclick="showImageModal('../uploads/job_photos/<?= htmlspecialchars($paths['original']) ?>')"
                     class="block aspect-square w-full text-left">
            ...
            <?php else: ?>
              <div class="flex justify-center">
                <div class="inline-flex items-center px-5 py-2.5 bg-gray-400 text-white rounded-lg font-medium cursor-not-allowed">
        ...
        <div class="bg-red-50 p-4 rounded-xl">
          <div class="space-y-3">
            ...
            <div class="border-t border-red-200 pt-2">
              <p class="text-sm text-red-600"><strong>ตีกลับโดย:</strong> <?= htmlspecialchars($job['returned_by_name']) ?></p>
            </div>
            <?php endif; ?>
            <?php if (!empty($job['returned_at'])): ?>
            <div>
              <p class="text-sm text-red-600"><strong>วันที่ตีกลับ:</strong> <?= date('d/m/Y H:i น.', strtotime($job['returned_at'])) ?></p>
            </div>
            <?php endif; ?>
            <?php if (!empty($job['revision_count']) && $job['revision_count'] > 0): ?>
            <div>
              <p class="text-sm text-red-600"><strong>จำนวนครั้งที่ตีกลับ:</strong> <?= $job['revision_count'] ?> ครั้ง</p>
            </div>
            <?php endif; ?>
          </div>
        </div>
      ...
      <!-- Summary Card -->
      <section class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 animate-slide-up" style="animation-delay: 0.4s;">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-lg font-semibold text-gray-900 mb-1">สรุปข้อมูลงาน</h3>
            <p class="text-sm text-gray-600">งาน #<?= $job_id ?> - <?= htmlspecialchars($job['contract_number']) ?></p>
          </div>
