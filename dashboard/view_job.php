...
<div class="max-w-6xl mx-auto p-3 sm:p-4 space-y-4 sm:space-y-6 pb-8">
          ...
          <div class="flex flex-col gap-2">
            <div class="flex items-center justify-between gap-2">
              <div class="flex-1 min-w-0">
                ...
                <p class="font-bold text-base sm:text-lg text-gray-900 truncate"><?= htmlspecialchars($job['contract_number']) ?></p>
              </div>
        ...
        <div class="info-box">
          ...
          <div class="mt-2">
            <?php
              ...
              if ($priority === 'urgent') echo '<span class="status-badge priority-urgent">🔴 ด่วนที่สุด</span>';
              elseif ($priority === 'high') echo '<span class="status-badge priority-high">🟠 ด่วน</span>';
              else echo '<span class="status-badge priority-normal">🟢 ปกติ</span>';
            ?>
          </div>
        </div>
      </div>
        ...
        <div class="info-box info-box-highlight">
          <div class="flex items-center justify-between gap-2">
            <div class="flex-1 min-w-0">
              ...
              <p class="font-bold text-base sm:text-lg text-gray-900 break-words"><?= htmlspecialchars($job['location_info']) ?></p>
            </div>
    ...
    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-xl">
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="space-y-2">
            <p class="text-red-700 break-words"><strong>เหตุผล:</strong> <?= nl2br(htmlspecialchars($job['return_reason'])) ?></p>
            <?php if (!empty($job['returned_by_name'])): ?>
              <p class="text-red-600 text-sm"><strong>ตีกลับโดย:</strong> <?= htmlspecialchars($job['returned_by_name']) ?></p>
            <?php endif; ?>
            <?php if (!empty($job['returned_at'])): ?>
              <p class="text-red-600 text-sm"><strong>วันที่ตีกลับ:</strong> <?= date('d/m/Y H:i น.', strtotime($job['returned_at'])) ?></p>
            <?php endif; ?>
            <?php if (!empty($job['revision_count']) && $job['revision_count'] > 0): ?>
              <p class="text-red-600 text-sm"><strong>จำนวนครั้งที่ตีกลับ:</strong> <?= $job['revision_count'] ?> ครั้ง</p>
            <?php endif; ?>
          </div>
        </div>
      </div>
  ...
  <form method="post" action="save_job.php" enctype="multipart/form-data" id="jobForm" class="space-y-4 sm:space-y-6">
    ...
    <div class="card p-4 sm:p-6 fade-in">
      ...
      <div class="mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <label class="radio-card">
            <input type="radio" name="result" value="พบผู้เช่า/ผู้ค้ำ/ผู้ครอบครอง" required class="sr-only" onchange="updateProgress()">
          ...
          <label class="radio-card">
            <input type="radio" name="result" value="ไม่พบผู้เช่า/ผู้คำ/ผู้ครอบครอง" required class="sr-only" onchange="updateProgress()">
            <div class="flex items-center gap-2">
              <div class="text-xl">❌</div>
              <div>
          ...
          <label class="radio-card">
            <input type="radio" name="result" value="พบที่ตั้ง /ไม่พบรถ" required class="sr-only" onchange="updateProgress()">
            <div class="flex items-center gap-2">
              <div class="text-xl">🏠</div>
              <div>
          ...
          <label class="radio-card">
            <input type="radio" name="result" value="ไม่พบที่ตั้ง/ไม่พบรถ" required class="sr-only" onchange="updateProgress()">
            <div class="flex items-center gap-2">
              <div class="text-xl">❓</div>
              <div>
          ...
          <label class="radio-card">
            <input type="radio" name="result" value="พบ บ.3 ฝากเรื่องติดต่อกลับ" required class="sr-only" onchange="updateProgress()">
            <div class="flex items-center gap-2">
              <div class="text-xl">📋</div>
              <div>
          ...
          <label class="radio-card">
            <input type="radio" name="result" value="พบรถ ไม่พบผู้เช่า/ผู้ค้ำ" required class="sr-only" onchange="updateProgress()">
            <div class="flex items-center gap-2">
              <div class="text-xl">🚗</div>
              <div>
      ...
      <!-- Images -->
      <div class="mb-6">
        <label class="block text-sm sm:text-base font-bold text-gray-900 mb-2 required">📷 รูปถ่าย (ต้องอัปโหลด 4 รูป)</label>
        <div class="file-upload" id="fileUploadArea">
          <input type="file" name="images[]" multiple accept="image/*" 
                 class="hidden" id="imageInput" onchange="handleFileSelection(this)">
      ...
      <div class="text-center space-y-4">
        <button type="submit" class="btn btn-success text-base sm:text-lg px-8 py-4 w-full sm:w-auto" id="submitBtn" disabled>
          💾 บันทึกผลภาคสนาม
        </button>
